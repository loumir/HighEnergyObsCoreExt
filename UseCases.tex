...
Observation datasets, typically in HE : event-lists and event bundle, response function datasets, and advanced datasets.
In agreement with Semantics WG we currently have  3 draft vocabularies : 
\begin{itemize} 
...
Note that the Radio astronomical community also requires the definition of new types of data products for their science ready datasets as «advanced dataset ». 
\end{itemize} 

Three strategies have been considered in this note to organize data discovery based on an extension of  ObsCore 1.1 in TAP. Each of them needs to be evaluated in various situations and explored further in terms of clarity for the user and robustness. 

\begin{itemize} 
\item {\bf Proposal 1}: Serve all data products  in the {\tt ivoa.obscore} table
The column 'dataproduct\_type' in {\tt ivoa.obscore} can contain any term from one of the three vocabularies  \textsl{product-type}, \textsl{advanced-product-type},  \textsl{response-type}: 
All products are distributed with the {\tt ivoa.obscore} table as in 
Chandra use case A.1.8, A.2.1,A.2.3, A2.4 , CTAO: use case A1.9, SWGO use case A1.3

Problem: If you search {\bf hea-event-list} or {\bf light-curve} in a sky region together with {\bf response-function} files, for instance {\bf psf}, the query response may provide all datasets together mixing the various data products type together. Therefore the user will have to sort which {\bf psf}applies to  which {\bf hea-event-list} or {\bf light-curve} for instance. \\
 You can use GROUP BY {\em obs\_id} for instance, but still there may be several light curves findable in the selected region, so the issue is to organize and group properly corresponding light curves with appropriate {\bf psf}, files, especially if the point spread function varies in the field of view. 

\item {\bf Proposal 2} : Add new terms to product-type vocabulary and distinguish among response types and advanced product types using {\em dataproduct\_subtype} in {\tt ivoa.obscore}. 
...
See examples given for KM3net  A1.10, A1.13, and Chandra A1.8

Problems: 
\begin{itemize} 
\item {\em dataproduct\_subtype} is not standardized in ObsCore and left open for archives’ specific needs. 
\item The response type needs  2 columns to be determined. 
\item The interpretation  of the {\em dataproduct\_subtype} column is ambiguous as the content can be a {\bf response} type, a category of map (excess map, significance map, etc) or some specific description chosen in a specific archive. 
\end{itemize}
...
\begin{itemize}
\item {\tt ivoa.obscore} uses {\em dataproduct\_type} in \textsl{product-type} vocabulary
\item {\tt ivoa.response} uses {\em dataproduct\_type} in \textsl{response-type} vocabulary
\item {\tt ivoa.adp} uses {\em dataproduct\_type} in \textsl{advanced-product-type} vocabulary
\end{itemize}

The benefits lies in tracing clearly the connections between a response file ( resp. advanced data products)  and a data set from the  current \textsl{product-type} datasets.
This strategy is still to investigate in detail. Appendix B provides hints for implementing it. 
\end{itemize} 
...
Then, for each row of the output, we get access to a DataLink table (in VOTable) describing associated data linked to the hea-event-list dataset using the {\em access\_url}' column value of the  ObsCore query response table.
...
\begin{enumerate}[(i)]
  ...
  \item for each row of the DataLink VOTable, get the {\em content\_qualifier} and the {\em access\_url} column's value,
...
The {\em obs\_publisher\_did} of the single discovered hea-event-list is repeated in the ID column of the DataLink table.
